task getScript {
	command {
		wget "https://raw.githubusercontent.com/manning-lab/topmed-t2d-glycemia-public/gxe_pilot/workflows/singleVariantAssociation/association_gxe.R"
	...
	if(defined(these_snps)) {
		...
		scatter(this_genotype_file in these_genotype_files) {
			...
			call assocTest as assocTestConditional {
				input: gds_file = this_genotype_file, null_file = fitNullConditional.model, label = this_label, test = this_test, mac = this_mac, script = getScript.assoc_script, memory = this_memory, disk = this_disk, ivars_string = this_ivars_string
			}
		}
