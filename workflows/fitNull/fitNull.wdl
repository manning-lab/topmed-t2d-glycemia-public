...
task fitNull {
	...
	command {
		R --vanilla --args ${genotype_file} ${phenotype_file} ${outcome_name} ${outcome_type} ${covariates_string} ${default="NA" conditional} ${sample_file} ${label} ${kinship_matrix} ${id_col} < ${script}
	}
...
task summary {
	...
	command {
		R --vanilla --args ${phenotype_file} ${outcome_name} ${covariates_string} ${default="NA" conditional} ${label} ${cohort_column} < ${script}
	}
...
workflow nullModel {
	...
	call fitNull {
            input: genotype_file = this_genotype_file, phenotype_file = this_phenotype_file, outcome_name = this_outcome_name, outcome_type = this_outcome_type, covariates_string = this_covariates_string, conditional = this_conditonal, sample_file = this_sample_file, label = this_label, kinship_matrix = this_kinship_matrix, id_col = this_id_col, script = getScript.script, memory = this_memory, disk = this_disk
	}

	call summary {
		input: phenotype_file = this_phenotype_file, outcome_name = this_outcome_name, covariates_string = this_covariates_string, conditonal = this_conditonal, label = this_label, cohort_column = this_cohort_column, script = getScript.summary_script, memory = this_memory, disk = this_disk
	}
