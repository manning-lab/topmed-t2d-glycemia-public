...
if(sum(maf.filt)==0) {
  ...
  if(outcomeType=="dichotomous" ) {
    if (test=="burden"){
      assoc <- assocTestSeq(genoData, nullmod, variant.groups, test=test, burden.test=pval)
    } else if (test == "skat"){
      assoc <- assocTestSeq(genoData, nullmod, variant.groups, test=test, pval.method=pval)
    } else {
      assoc <- c()
