...
task summary {
	...
	command {
		R --vanilla --args ${ped_file} ${outcome} ${covars} ${default="NA" conditional_string} ${default="NA" ivar_string} ${default="NA" group_var} ${label} ${cohort_column} ${sex_column} < ${script}
	}
...
workflow phenotypeSummary {
	...
	call summary {
            input: ped_file = this_ped_file, outcome = this_outcome, covars = this_covars, conditional_string = this_conditional_string, ivar_string = this_ivar_string, group_var = this_group_var, label = this_label, cohort_column = this_cohort_column, sex_column = this_sex_column, script = getScript.script, memory = this_memory, disk = this_disk
	}
}
